...
class LogStash::Outputs::Logan < LogStash::Outputs::Base
  ...
  def register
    ...
    #     #else
    #       #raise LogStash::ConfigurationError, "error parsing chunk_limit_size"
    #     end
    ...
    #       raise LogStash::ConfigurationError, "chunk_limit_size must be between 1MB and 4MB"
    #     end
    #   end
    # end
  ...
  def multi_receive_encoded(events_encoded)
    ...
    begin
      ...
      begin
        ...
        incoming_records_per_tag,invalid_records_per_tag,tag_metrics_set,logGroup_labels_set,tags_per_logGroupId,lrpes_for_logGroupId = group_by_logGroupId(events_encoded)
  ...
  # takes a LogStash event block and converts it to an in-memory zipfile, populating metrics hash provided
  # Any exception raised is passed into the metrics hash, to be re-thrown from multi_receive_encoded()

  # returns a Metadata key and value pair for upload_log_events_file, merely descriptive
  def getCollectionSource(input)
    collections_src = []
    if !is_valid(input)
      collections_src.unshift("source:LogStash")
    else
